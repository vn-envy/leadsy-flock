...
- **Veo wait-in-Inka was the timeout bug, not Veo itself.** Inka now *starts* the Veo LRO (seconds) and returns. A sidecar step `inka_harvest` polls `operations.get` once per worker invocation, sleeps `HARVEST_POLL_SECONDS` (12), and republishes itself (max 24). If Veo RAI-filters people/face (common when ASSET refs imply a client), harvest starts the next fallback (9:16 no refs, then 16:9) instead of stopping on `done_no_bytes`. It never `publish_next`s and never marks the campaign `completed`. Stella always emits `/media/{id}/clip` and `/jingle`; the page polls `GET /media/{id}/ready` until harvest writes GCS.
...
- Asset kit ratios live in [design.md](design.md). One Veo 8s 9:16 (room tone) master + ffmpeg crops cover Feed / square / Stories. Harvest muxes **English (`clip-en`) and local-language (`clip-indic`) VO**, plus captioned cuts. No IAB GIF pack, no zip, no autopost. Live proof: fictional **Noya Salon** `noya-salon-e548bd87` (own photos → Veo refs, dual VO) at `GET /k/{id}`.
- **Own visuals first.** Scout + the brief (`website`, listing, menu, `assetUris`) feed `app/own.py`. Gemini image is a refine/filler; it generates a shop from scratch only when nothing usable arrives. Live origin on `noya-salon-e548bd87`: `own` / 3 frames. Image refine 429 kept the raw owner bytes.
- Stella landings use locked tokens from [design.md](design.md). Scout names a `themeId`; orange brand paint cannot become the page background. Salons/clinics/cafés map to `paper`.
