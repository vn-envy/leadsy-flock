...
- Asset kit ratios live in [design.md](design.md). One Veo 16:9 master + centre crops (ffmpeg on the worker image) cover Feed / square / Stories — we do not run a second Veo LRO. No IAB GIF pack, no zip, no autopost. Live proof: fictional **Noya Salon** `noya-salon-7384033d` (`paper` theme) at `GET /k/{id}`.
